...
export default function TuteeCourseEnrollment() {
  ...
  const handleUnenroll = async (id) => {
    if (!window.confirm("Chắc chắn hủy?")) return;
    setActionLoading(true);
  ...
  const filteredCourses = courses.filter((c) => {
    ...
    if (filterStatus === "open")
      return match && c.status === "open" && !c.isEnrolled;
    return match;
  });
        ...
        <div className="flex gap-2">
          {["all", "open", "enrolled"].map((s) => (
            <button
              ...
              className={`px-4 py-2 rounded-lg ${filterStatus === s ? "bg-blue-600 text-white" : "bg-gray-100 text-gray-700"}`}
            >
